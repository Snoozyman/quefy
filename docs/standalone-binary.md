...
*ffmpeg on macOS via Homebrew is larger. Use a static build instead.

## Limitations

- **Native modules** — Bun compile may fail with `sharp` (image processing). If it does, disable image optimization at build time
