export function parseYtDlpError(stderr: string): string {
  ...
  if (lower.includes('http error 403') || lower.includes('forbidden')) {
    return 'Access denied (403). The video may require authentication. Upload fresh YouTube cookies.'
  }

  if (lower.includes('http error')) {
    const match = stderr.match(/http error (\d+)/i)
    const code = match ? match[1] : '?'
    return `YouTube returned HTTP ${code}. The cookies may be expired or invalid. Try re-exporting them.`
  }
