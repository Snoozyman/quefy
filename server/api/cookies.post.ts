...
export default defineEventHandler(async (event) => {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    const file = body?.find(
      (p) => p.name === 'file' || p.name === 'cookies' || p.filename
    )
    if (!file || !file.data) {
    ...
    if (typeof body === 'string') {
      ...
      throw createError({
        statusCode: 400,
        statusMessage: 'Send cookies as text/plain or JSON { cookies: "..." }'
      })
    }
  }
  ...
  if (!isValidNetscapeFormat(content)) {
    throw createError({
      statusCode: 400,
      statusMessage:
        'Invalid cookies format. Expected Netscape-format cookies (tab-separated fields).'
    })
  }
  ...
  try {
    ...
    verifyError =
      'Cookies saved but verification failed. They may be expired or in the wrong format.'
  }
