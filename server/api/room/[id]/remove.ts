...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.songId || !body?.hostToken) {
    throw createError({ statusCode: 400, statusMessage: "Missing songId or hostToken" });
  }

  if (!verifyHost(id, body.hostToken)) {
    throw createError({ statusCode: 403, statusMessage: "Only the host can remove songs" });
  }

  const ok = removeFromQueue(id, body.songId, body.hostToken);
  if (!ok) throw createError({ statusCode: 404, statusMessage: "Song not found" });
