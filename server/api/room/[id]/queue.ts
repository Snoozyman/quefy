...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.videoId) throw createError({ statusCode: 400, statusMessage: "Missing videoId" });

  let audio;
  try {
    audio = await getAudioStreamUrl(body.videoId);
  } catch (err: any) {
    throw createError({ statusCode: 422, statusMessage: err.message || "Failed to fetch video" });
  }

  const song = addToQueue(id, body.videoId, audio.title, audio.url, body.addedBy || "");
  if (!song) throw createError({ statusCode: 500, statusMessage: "Failed to add song" });
