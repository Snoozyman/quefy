...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.hostToken) throw createError({ statusCode: 400, statusMessage: "Missing hostToken" });

  if (!verifyHost(id, body.hostToken)) {
    throw createError({ statusCode: 403, statusMessage: "Only the host can control playback" });
  }
