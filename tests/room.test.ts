...
describe("addToQueue", () => {
  ...
  test("defaults addedBy to Anonymous", () => {
    const room = createRoom();
    const song = addToQueue(room.id, "abc", "Test", "url", "");
    expect(song!.addedBy).toBe("Anonymous");
  });

  test("returns null for non-existent room", () => {
    const song = addToQueue("NOPE", "abc", "Test", "url", "Alice");
...
describe("removeFromQueue", () => {
  test("removes a song by id", () => {
    const room = createRoom();
    const song = addToQueue(room.id, "abc", "Test", "url", "Alice")!;
...
describe("togglePlay", () => {
  test("starts playing when queue has songs", () => {
    const room = createRoom();
    addToQueue(room.id, "abc", "Song A", "url", "Alice");
...
describe("skipSong", () => {
  test("advances to the next song in queue", () => {
    const room = createRoom();
    addToQueue(room.id, "abc", "Song A", "url", "Alice");
    addToQueue(room.id, "def", "Song B", "url", "Bob");
