  ...
  <div class="search-container relative">
    ...
    >
      ...
      >
        ...
        <UIcon
          :name="r.source === 'spotify' ? 'i-simple-icons-spotify' : 'i-simple-icons-youtube'"
...
function extractVideoId(input: string): string | null {
  ...
  const match = trimmed.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([\w-]+)/)
...
function onInput() {
  ...
  searchTimer = setTimeout(async () => {
    const endpoint = mode.value === 'youtube'
...
function highlightPrev() {
  if (!results.value.length) return
  highlightIdx.value = (highlightIdx.value - 1 + results.value.length) % results.value.length
}
...
function addHighlighted() {
  ...
  const r = idx >= 0 && idx < results.value.length
    ? results.value[idx]
    : results.value[0]
  if (r) selectResult(r)
}
